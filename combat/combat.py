# -*- coding: utf-8 -*-
"""
The Combat module is an integral component of this COMBAT package, designed to facilitate comprehensive model creation, aggregation, and stacking techniques. 
With a focus on optimizing predictive performance through strategic model combinations and ensemble methods, this module empowers users to harness the collective strength of multiple models for enhanced predictive accuracy across diverse datasets and tasks.

Functions within the Combat module:

1. `IsModelValid(model, coef_expectation, p_value, metric, gini_cutoff, auc_cutoff)` -  evaluate the validity and performance of individual models through rigorous testing and validation procedures. 

2. `ModelCombination(y_train, x_train, y_test, x_test, max_model_number
                    , dependent_number, coef_expectation, intercept, penalty
                    , alpha, p_value, check_sample, metric, gini_cutoff, auc_cutoff)` - implement techniques for generating and validating multiple models effectively. 

3. `ModelMetaInfo(models_dict, sort_by)` - access meta-information associated with models, including details on performance metrics.

4. `WeightsBagging(models_dict, metric, check_sample)` - perform aggregation of multiple models to leverage their collective predictive capabilities

5. `PredictionBagging(models_dict, weights_dict, x_data)` - aggregate predictions generated by multiple models to produce a consolidated prediction output              

6. `ModelStacking(models_dict, x_data, y_data, penalty, alpha, fit_intercept)` - employ model stacking techniques to create powerful ensemble models

7. `PredictionStacking(models_dict, x_data, model)` - generate stacked predictions by applying trained meta-learners to the predictions of base models

8. `AggregationMetaInfo(models_dict, x_train, y_train, x_test, y_test)` - access meta-information associated with model aggregation and stacking processes

The Combat module serves as a versatile toolkit for model validation, aggregation, and stacking, 
empowering users to harness the collective predictive strength of multiple models and optimize predictive performance across diverse datasets and applications
"""
...
def AggregationMetaInfo(
    ...
    for metric in ['gini', 'auc', 'f1', 'brier']:
        for sample in ['test', 'train']:
            ...
            weights = WeightsBagging(models_dict = models_dict
                                       , metric = metric
                                       , check_sample = sample
                                       )
            
            pred = PredictionBagging(models_dict = models_dict
